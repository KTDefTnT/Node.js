...
app.use(async (ctx, next) => {
  ...
  if (url === '/login' || url === '/favicon.ico') {
    ...
    console.log('cookies', Object.keys(cookies), CONFIG.key);
    if (!Object.keys(cookies).includes(CONFIG.key)) {
      ctx.body = {
        message: '您还未登录，请先登录！'
      }
    } else {
...
router.post('/login', async (ctx, next) => {
  ...
  let key = ctx.session._sessCtx.externalKey;
  // 在redis中保存数据
  // 在redis中设置sid的值
...
router.get('/users/getUser', async (ctx, next) => {
  ...
  ctx.body = {
    ...
    // userInfo从redis中获取
    userInfo: await client.get(cookies[CONFIG.key])
  }
});
...
router.post('/logout', async (ctx, next) => {
  ...
  const { header } = ctx.request;
  const cookies = seralize(header.cookie);
  await client.del(cookies[CONFIG.key]);
  ctx.body = {
    message: '登出成功！'
  };
  next();
